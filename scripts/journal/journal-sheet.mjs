...
// NOTE: Keeping this in case we need to fix for V12 or revert, for some reason
...
export default class VessayaJournalSheet extends foundry.applications.sheets.journal.JournalEntrySheet {
	...
	async _onRender(context, options) {
		...
		let editButton = $(thisEl).find(".document-title-edit")
		...
		$(thisEl).find(".window-title").after(
			`<a data-action="edit-document-title" class="document-title-edit"><i class="fa-solid fa-edit"></i></a>`
		)

		thisEl.querySelector(".document-title-edit").addEventListener("click", (event) => {
			const ct = event.currentTarget
			$(ct).closest(".journal-sheet.vessaya").find("input.title").toggleClass("display")
		})
	}
